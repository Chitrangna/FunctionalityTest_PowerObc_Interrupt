...
void TWI_init_slave(void)
{
	...
	//TWCR = (1<<TWEA)|(1<<TWEN)|(0<<TWSTO)|(0<<TWSTA)|(1<<TWIE);
...
{
	char recv_data;
	TWCR=(1<<TWINT)|(1<<TWEN)|(1<<TWIE);    // Clear TWI interrupt flag,Enable TWI
	while (!(TWCR & (1<<TWINT))); // Wait till complete TWDR byte transmitted
...
uint8_t TWI_read_data_slave(void)
{
	begin:
		...
		uint8_t send;	//send 0x80 wale switch k andar daalke check: chalna chahiye		 
		send=TWDR;
		return send;
}
